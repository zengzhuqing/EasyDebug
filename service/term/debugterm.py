...
class Multiplex:
	def __init__(self):
		...
		self.thread.start()
	def create(self,w=80,h=25,crash_cmd=None):
		pid,fd=pty.fork()
		if pid==0:
			try:
		...
		if req.environ['PATH_INFO'].endswith('/u'):
			...
			if s in self.session:
				...
				if not (w>2 and w<256 and h>2 and h<100):
					w,h=80,25
				term=self.session[s]=self.multi.create(w,h,crash_cmd)
			if k:
...
#	parser.add_option("-c", "--command", dest="cmd", default=None,help="set the command (default: /bin/login or ssh localhost)")
	...
	try:
		wsgiref.simple_server.make_server('0.0.0.0', int(o.port), at).serve_forever()
